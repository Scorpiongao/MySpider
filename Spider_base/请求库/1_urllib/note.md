...
- urlopen 的返回对象
    - 案例v3
    - HTTPResponse类型对象
        - read(): 读取信息，**bytes类型，需要解码**
        - geturl: 返回请求对象的url
        - info: 请求反馈对象的meta信息
        - getcode: 返回的http code
        - getheaders: 返回响应头信息
        - getheader(name): 返回响应头中的具体值     

- request.data 的使用
    - 访问网络的两种方法
        - get: 
            - 利用参数给服务器传递信息 `urllib.request.urllib.urlopen(url)`
            - 参数为dict，然后用parse编码(parse.urlencode(data))，拼接在base_url上
            - 案例v4
            - 另一种解决中文乱码方式 `urllib.parse.quote(keyword)`
        - post
            - 一般向服务器传递参数使用 `urllib.request.urlopen(url,data=data)`
            - post是把信息自动加密处理
            ...
            - 需要利用request.Request 类 `urllib.request.Request(url=baseurl, data=data, headers=headers)`
                - 请求成一个独立的对象
                - 更加丰富灵活的配置参数
            - 案例V6
...
- ProxyHandler处理（代理服务器）
    ...
    - 基本使用步骤：
        ...
        - 也可在3的基础上直接使用 opener.open(url)函数访问网页
    - 案例v10
...
- 使用cookie登录
    ...
    - http模块包含一些关于cookie的模块，通过他们我们可以自动使用cookie
        ...
        - MozillaCookieJar(filename, delayload=None, policy=None):
            - 创建与moczilla浏览器cookie.txt兼容的FileCookieJar实例

'''
GET songs or mvs' comments
'''
from Crypto.Cipher import AES
import base64
import requests
import json
from singer_id import SingerId
from db import MongoDB
from config import *
import pymongo
from singer import Singer
from song_id import SongId,MvId
import time
import random

class Comment():
    def __init__(self,id,type):
        self.id=id
        self.type=type

        self.headers={
                'Host': 'music.163.com',
                'Referer': 'http://music.163.com/search/',
                'User-Agent': 'Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/65.0.3325.32 Safari/537.36'
        }


    # first_param = '{rid:"", offset:"20", total:"false", limit:"20", csrf_token:""}'
        self.second_param = "010001"
        self.third_param = ""
        self.forth_param = "0CoJUm6Qyw8W8jud"

    def get_first_params(self,page):
        if page==0:
            first_param ='{rid:"", offset:"0", total:"true", limit:"20", csrf_token:""}'
            return first_param
        else:
            first_param ='{rid:"", offset:"%s", total:"false", limit:"20", csrf_token:""}'%(page*20)
            return first_param

    def get_params(self,page):
        iv = "0102030405060708"
        first_key = self.forth_param
        second_key = 16 * 'F'
        h_encText = self.AES_encrypt(self.get_first_params(page), first_key, iv)
        h_encText = self.AES_encrypt(h_encText, second_key, iv)
        return h_encText


    def get_encSecKey(self):
        encSecKey = ""
        # encSecKey =''
        return encSecKey


    def AES_encrypt(self,text, key, iv):
        pad = 16 - len(text) % 16
        if isinstance(text, bytes):
            text = text.decode('utf-8')
        text = text + str(pad * chr(pad))
        encryptor = AES.new(key, AES.MODE_CBC, iv)
        encrypt_text = encryptor.encrypt(text)
        encrypt_text = base64.b64encode(encrypt_text)
        return encrypt_text


    def get_json(self, params, encSecKey):
        if self.type=='song':
            url = "http://music.163.com/weapi/v1/resource/comments/R_SO_4_{song_id}/?csrf_token=".format(song_id =self.id )
        elif self.type == 'mv':
            url="https://music.163.com/weapi/v1/resource/comments/R_MV_5_{song_id}/?csrf_token=".format(song_id =self.id )
        else:
            print('type error',self.type )
        data = {
            "params": params,
            "encSecKey": encSecKey
        }
        response = requests.post(url, headers=self.headers, data=data)
        return response.text

    def get_hotcomment(self,json_text):
        if json_text :
            if json_text.get('hotComments'):
                for item in json_text['hotComments']:
                    hotComment={}
                    hotComment ['commentId']=item ['commentId']
                    hotComment ['content']=item ['content']
                    hotComment ['likedCount']=item ['likedCount']
                    hotComment ['time']=item ['time']
                    hotComment ['avatarUrl']=item ['user']['avatarUrl']
                    hotComment ['nickname']=item ['user']['nickname']
                    hotComment ['userId']=item ['user']['userId']
                    hotComment ['vipType']=item ['user']['vipType']
                    print(hotComment )
                    yield hotComment

    def get_total(self):
        json_text=self.get_json_text(0)
        print(json_text)
        total = int(json_text['total'])
        return total

    def get_json_text(self,page):
        params = self.get_params(page)
        print(params)
        encSecKey = self.get_encSecKey()
        json_text = self.get_json(params, encSecKey)
        print(json_text)
        json_dict = json.loads(json_text)
        return json_dict

    def get_comments(self,json_text):
        if json_text :
            if json_text.get ('comments'):
                for item in json_text['comments']:
                    comment = {}
                    comment['commentId'] = item['commentId']
                    comment['content'] = item['content']
                    comment['likedCount'] = item['likedCount']
                    comment['time'] = item['time']
                    comment['avatarUrl'] = item['user']['avatarUrl']
                    comment['nickname'] = item['user']['nickname']
                    comment['userId'] = item['user']['userId']
                    comment['vipType'] = item['user']['vipType']
                    print(comment)
                    yield comment


    def get_all_comments(self):
        total = self.get_total()
        for page in range(total // 20 + 1):
            json_text=self.get_json_text(page)
            comment=[comment for comment in self.get_comments(json_text)]
            return comment

    def result_data(self,singerid,singername ,song):
        if self.type=='song':
            comments['songid']=self.id
            comments ['song']=song
            comments ['singername']=singername
            comments ['singerId']=singerid

        if self.type=='mv':
            comments['mvid'] = self.id
            comments['mv'] = song
            comments['singername'] = singername
            comments['singerId'] = singerid

        json_text = self.get_json_text(0)
        comments['hotcomments'] =[comment for comment in self.get_hotcomment(json_text)]

        print(comments )
        self.save_to_mongo(comments)

    def save_to_mongo(self,result):
        if self.type =='song':
            print("Those are songs'comments" )
            if db[MONGO_TABLE1 ].update({'songid':result ['songid']},{'$set':result },True ):
                print('save to mongodb successfully',result ['songid'])
            else:
                print('save to mongodb failed', result['songid'])
        if self.type =='mv':
            print("Those are mvs'comments" )
            if db[MONGO_TABLE3 ].find_one({'mvid':result ['mvid']}):
                print('This mv is already in mongodb!',result['mvid'])
            else:
                if db[MONGO_TABLE3 ].update({'mvid':result ['mvid']},{'$set':result },True ):
                    print('save to mongodb successfully',result ['mvid'])
                else:
                    print('save to mongodb failed', result['mvid'])

    def run(self,id,singername ,song):

        self.result_data(id,singername ,song)


def main(type):
    if type=='song':
        songs=SongId ()
        for data in songs.get_song_id():
            song_id=data['song_id']
            singerId=data['singerId']
            singername=data['singername']
            song=data['song']

            print(song_id ,singerId ,singername ,song )
            #判断数据库中是否已经存在，防止重复爬取，减少被封的风险
            if db[MONGO_TABLE1].find_one({'songid':song_id}):
                print('%s already in mongodb'%song_id ,song)
            else:
                app = Comment(song_id,type)
                app.run(singerId ,singername ,song )
                time.sleep(random .randint (1,4))

    if type=='mv':
        songs = MvId()
        for data in songs.get_mv_id():
            mv_id = data['mv_id']
            singerId = data['singerId']
            singername = data['singername']
            song = data['song']

            print(mv_id, singerId, singername, song)
            # 判断数据库中是否已经存在，防止重复爬取，减少被封的风险
            if db[MONGO_TABLE3].find_one({'mvid': mv_id}):
                print('%s already in mongodb' % mv_id, song)
            else:
                app = Comment(mv_id,type)
                app.run(singerId, singername, song)
                time.sleep(random.randint(1, 4))





if __name__ == "__main__":
    comments = {}
    client = pymongo.MongoClient(MONGO_URL, connect=False)
    db = client[MONGO_DB]
    comments = {}
    # song_id='65800'#song id
    # app = Comment(song_id)
    # app.run(singerId, singername, song)
    # app.run('2116','陈奕迅','最佳损友')
    type='mv'
    main(type)
